...
internal static class LobbyPatcher
{
    ...
    internal static LobbyPatcherChanges GetLobbyDiff(Lobby lobby1, Lobby lobby2)
    {
        ...
        // If the data has been removed in the new version
        if (newLobby.Data == null)
        {
            ...
            // If both new and old lobby data is null, no need to trigger the data remove change
        }
        else
        {
        ...
        if (oldLobbyPlayersIdxById.Count != 0)
        {
            ...
            for (int playerIdx = oldLobby.Players.Count - 1; playerIdx >= 0; playerIdx--)
            {
                ...
                if (oldLobbyPlayer.LastUpdated != newLobbyPlayer.LastUpdated)
                    changes.PlayerLastUpdatedChange(newLobbyPlayerIdx, newLobbyPlayer.LastUpdated);

                if (oldLobbyPlayer.ConnectionInfo != newLobbyPlayer.ConnectionInfo)
